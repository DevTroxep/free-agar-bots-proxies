    WebSocket = require('ws'),{ murmur2 } = require('murmurhash-js'),
    ...
    proxies = require('fs').readFileSync('./http_proxies.txt').toString().split('\n').map(r => r.replace('\r', '')).filter(l => !!l);
...
const dataBot = {
    ...
    connect() {
        ...
        const IP = proxies[~~(Math.random() * proxies.length)];
        this.ws = new WebSocket(game.url, { agent: new HttpsProxyAgent("http://" + IP) });
